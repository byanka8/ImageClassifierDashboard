...
class TrainingStreamServicer(training_pb2_grpc.TrainingStreamServicer):
    ...
    #     """Unary RPC: client sends LossUpdate(loss=float, iteration=int)"""
    #     global latest_loss
    #     try:
    #         with state_lock:
    ...
    #         return training_pb2.Ack(status="ok")
    #     except Exception as e:
    #         logging.exception("Error in SendLossUpdate")
    #         return training_pb2.Ack(status=f"error: {e}")
...
# def update_once():
#     """Called each frame in the DearPyGUI loop to refresh UI from latest state."""
#     global last_time, frame_count, fps_timer, loss_history
...
#     with state_lock:
#         batch_copy = list(latest_batch) if latest_batch is not None else None
#         loss_val = latest_loss

#     # --- update images and text widgets ---
#     if batch_copy:
#         n = min(len(batch_copy), TOTAL)
#         for i in range(n):
#             img_data = batch_copy[i]
#             try:
#                 pil_img = Image.open(io.BytesIO(img_data.data))
#                 flat = pil_to_rgba_flat(pil_img, target_size=IMG_SIZE)
#                 dpg.set_value(texture_ids[i], flat)
#             except Exception as e:
#                 logging.exception("Failed to decode/update image %d: %s", i, e)
#             try:
#                 dpg.set_value(pred_widgets[i], f"Pred: {img_data.prediction}")
#                 dpg.set_value(gt_widgets[i], f"GT: {img_data.ground_truth}")
#             except Exception:
#                 pass
...
#                 blank = [0.0] * (IMG_SIZE * IMG_SIZE * 4)
#                 dpg.set_value(texture_ids[i], blank)
#                 dpg.set_value(pred_widgets[i], "")
#                 dpg.set_value(gt_widgets[i], "")
...
# frame_counter = 0
# UPDATE_CHART_EVERY_N_FRAMES = 3

# def update_once():
#     global frame_counter, frame_count, last_time, loss_history, fps_timer
...
#     # --- update images and text only if new batch arrived ---
#     if batch_copy:
...
#     # --- update loss chart only every N frames ---
#     frame_counter += 1
#     if frame_counter % UPDATE_CHART_EVERY_N_FRAMES == 0:
#         loss_history.append(loss_val)
#         if len(loss_history) > 200:
#             loss_history = loss_history[-200:]
#         x_data = list(range(len(loss_history)))
